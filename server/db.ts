...
import { createEmptyDatabase, getDatabaseStore } from "./db/persistence";
...
export interface PlatformSettingsRecord {
  ...
  updatedAt: string;
  updatedBy?: string;
  // ---- Phase 3 — admin-configurable production + plans (persisted) ----
  planConfigs?: ProductionPlanConfigRecord[];
  productionConfig?: ProductionConfigRecord;
}
...
export interface CreativeRequestRecord {
  ...
  createdAt: string;
  // ---- Phase 1 foundation (workspace-scoped creative requests) ----
  projectId?: string;
  requesterId?: string;
  requestType?: string;
  title?: string;
  ...
  assignedStudio?: 'artwork' | 'content' | 'motion' | 'lyrics' | 'epk' | 'document' | 'brand_asset' | 'template_engine' | 'export_engine';
  ...
  deliverableIds?: string[];
  updatedAt?: string;
  // ---- Phase 3 — internal production (private to admin surfaces) ----
  revisions?: CreativeRequestRevisionRecord[];
  deliveries?: CreativeRequestDeliveryRecord[];
}

/** One revision request against a creative request. */
export interface CreativeRequestRevisionRecord {
  id: string;
  requestId: string;
...
/** One delivery dispatch — produces real assets in the customer's project/library. */
export interface CreativeRequestDeliveryRecord {
  id: string;
  requestId: string;
  assetIds: string[];
  note?: string;
...
// ==========================================
// PRODUCTION JOB ENGINE RECORDS (Lifecycle: Submission -> Delivery)
// Generic production model supporting every Studio Service
// ==========================================
...
export interface DeliverableCommentRecord {
  id: string;
  authorId: string;
  authorName: string;
  authorRole: 'client' | 'producer' | 'admin';
  content: string;
  createdAt: string;
}

export interface ProductionDeliverableVersionRecord {
  id: string;
  ...
  uploadedBy: string;
  uploadedAt: string;
  status: 'internal' | 'internal_review' | 'client_review' | 'revision_requested' | 'approved' | 'delivered';
  comments?: DeliverableCommentRecord[];
}
...
export interface ProductionDeliverableRecord {
  ...
  versions: ProductionDeliverableVersionRecord[];
}

export interface ProductionRevisionRecord {
  id: string;
  jobId: string;
  deliverableId?: string;
...
export interface ProductionFinalDeliveryRecord {
  deliveredAt: string;
  deliveredBy: string;
  deliveredByName: string;
  note?: string;
...
export interface ProductionJobRecord {
  ...
  internalNotes?: string;
  clientVisibleNotes?: string;
  revisions: ProductionRevisionRecord[];
  approvals: ProductionApprovalRecord[];
  finalDelivery?: ProductionFinalDeliveryRecord;
  createdAt: string;
  updatedAt: string;
}

// ==========================================
// PHASE 3 — ADMIN CONFIGURABLE PRODUCTION + PLANS
// ==========================================

export interface ProductionPlanConfigRecord {
  plan: string;
  label: string;
  blurb: string;
  storageMB: number;
...
export interface ProductionStudioConfigRecord {
  studio: string;
  label: string;
  description: string;
  categories: string[];
...
export interface ProductionConfigRecord {
  studios: ProductionStudioConfigRecord[];
  priorities: string[];
  requestCategories: string[];
  assetDefinitionHints: { category: string; count: number }[];
  revisionLimits: { plan: string; limit: number }[];
}

export const DEFAULT_PRODUCTION_PLAN_CONFIGS: ProductionPlanConfigRecord[] = [
  { plan: "artist_free", label: "Artist Free", blurb: "Workspace, projects, basic tools.", storageMB: 2048, generations: 20, exports: 20, productionRequests: 3, revisions: 2, capabilities: ["basic_workspace", "project_creation", "asset_library", "basic_creative_tools"] },
  { plan: "artist_pro", label: "Artist Pro", blurb: "Release builder, packages, motion, EPK, bulk export.", storageMB: 51200, generations: 1000, exports: 1000, productionRequests: 100, revisions: 5, capabilities: ["basic_workspace", "project_creation", "asset_library", "basic_creative_tools", "release_builder", "advanced_templates", "motion_tools", "epk_tools", "bulk_export", "secure_sharing", "premium_templates", "advanced_storage"] },
  { plan: "brand_free", label: "Brand Free", blurb: "Workspace, library, basic documents.", storageMB: 2048, generations: 20, exports: 20, productionRequests: 3, revisions: 2, capabilities: ["basic_workspace", "project_creation", "asset_library", "basic_creative_tools"] },
  { plan: "brand_pro", label: "Brand Pro", blurb: "Advanced library, documents, sharing, collaboration.", storageMB: 51200, generations: 1000, exports: 1000, productionRequests: 100, revisions: 5, capabilities: ["basic_workspace", "project_creation", "asset_library", "basic_creative_tools", "advanced_templates", "advanced_documents", "bulk_export", "secure_sharing", "team_collaboration", "premium_templates", "advanced_storage"] },
  { plan: "team", label: "Team (soon)", blurb: "Shared seats + review flows.", storageMB: 204800, generations: 5000, exports: 5000, productionRequests: 500, revisions: 8, capabilities: [] },
  { plan: "agency", label: "Agency (soon)", blurb: "Multi-client production.", storageMB: 512000, generations: 20000, exports: 20000, productionRequests: 2000, revisions: 10, capabilities: [] },
  { plan: "label", label: "Label (soon)", blurb: "Roster-wide creative ops.", storageMB: 512000, generations: 20000, exports: 20000, productionRequests: 2000, revisions: 10, capabilities: [] },
  { plan: "enterprise", label: "Enterprise (soon)", blurb: "SSO, audit, dedicated capacity.", storageMB: 2097152, generations: 100000, exports: 100000, productionRequests: 10000, revisions: 15, capabilities: [] },
];

export const DEFAULT_PRODUCTION_CONFIG: ProductionConfigRecord = {
  studios: [
    { studio: "artwork", label: "Artwork Studio", description: "Covers, artwork systems, visual identity.", categories: ["artwork", "cover", "album art"] },
    { studio: "content", label: "Content Studio", description: "Social packages, posts, captions, short-form.", categories: ["social", "content", "package"] },
    { studio: "motion", label: "Motion Studio", description: "Visualizers, animation, video edits.", categories: ["motion", "visualizer", "video", "animation"] },
    { studio: "lyrics", label: "Lyrics Studio", description: "Lyric videos, lyric sheets.", categories: ["lyrics"] },
    { studio: "epk", label: "EPK Studio", description: "Press kits, EPK dossiers.", categories: ["epk", "press kit"] },
    { studio: "document", label: "Document Studio", description: "Contracts, proposals, invoices, NDAs.", categories: ["document", "contract", "proposal", "invoice", "nda", "agreement"] },
    { studio: "brand_asset", label: "Brand Asset Studio", description: "Logos, brand systems, guidelines.", categories: ["brand", "logo", "brand asset"] },
    { studio: "template_engine", label: "Template Engine", description: "Template-driven asset generation.", categories: ["template"] },
    { studio: "export_engine", label: "Export Engine", description: "Bulk export and delivery packaging.", categories: ["export", "bulk export"] },
  ],
  priorities: ["low", "medium", "high", "urgent"],
  requestCategories: ["artwork", "social package", "visualizer", "EPK", "release package", "presentation", "brand asset", "business document", "custom creative work"],
  assetDefinitionHints: [
    { category: "SOCIAL", count: 34 }, { category: "STREAMING", count: 14 }, { category: "MOTION", count: 12 },
    { category: "PRINT", count: 10 }, { category: "PRESS", count: 9 }, { category: "MERCH", count: 8 },
    { category: "DIGITAL", count: 8 }, { category: "COPY", count: 10 },
  ],
  revisionLimits: [
    { plan: "artist_free", limit: 2 }, { plan: "artist_pro", limit: 5 },
    { plan: "brand_free", limit: 2 }, { plan: "brand_pro", limit: 5 },
    { plan: "team", limit: 8 }, { plan: "agency", limit: 10 }, { plan: "label", limit: 10 }, { plan: "enterprise", limit: 15 },
  ],
};
...
export interface BusinessDocumentRecord {
  id: string;
  workspaceId: string;
  documentType: 'proposal' | 'quotation' | 'company_letter' | 'invoice' | 'receipt' | 'company_profile' | 'business_letter';
  title: string;
...
export interface DatabaseSchema {
  ...
  business_documents: BusinessDocumentRecord[];
  ...
  production_jobs: ProductionJobRecord[];
  platform_settings: PlatformSettingsRecord;
}

// -----------------------------------------------------------------------------
// Password hashing — scrypt KDF (salted, memory-hard).
// Legacy SHA-256 hashes with a static salt are intentionally NOT accepted.
// -----------------------------------------------------------------------------
...
function scryptDerive(
  ...
  return crypto.scryptSync(password, salt, keyLength, { N, r, p, maxmem: 128 * 1024 * 1024 });
}

function hashPassword(password: string): string {
  const salt = crypto.randomBytes(16);
  const derived = scryptDerive(password, salt, KEY_LENGTH, SCRYPT_N, SCRYPT_R, SCRYPT_P);
  return `scrypt$${SCRYPT_N}$${SCRYPT_R}$${SCRYPT_P}$${salt.toString("base64")}$${derived.toString("base64")}`;
}

function verifyPasswordWithHash(password: string, stored: string): boolean {
  ...
  try {
    ...
    const derived = scryptDerive(password, salt, expected.length, N, r, p);
...
// Session tokens: only a one-way hash of the opaque token is stored at rest.
function hashSessionToken(token: string): string {
  return crypto.createHash("sha256").update(token).digest("hex");
}

function generateSessionToken(): string {
  return "kdh_sess_" + crypto.randomBytes(32).toString("hex");
}
...
class Database {
  ...
  /**
   * Async bootstrap required for the PostgreSQL engine.
   * The server calls this before it begins accepting requests.
   */
  public async initialize(): Promise<void> {
    const store = getDatabaseStore();
    if (store.kind === "postgres") {
      console.log(`[DB] Initializing PostgreSQL store (${store.label})…`);
      await store.initialize();
      this.data = store.load();
      console.log(
        `[DB] PostgreSQL store ready — ${this.data.users.length} user(s), ${this.data.workspaces.length} workspace(s).`
      );
    } else {
      console.warn(`[DB] Active store: ${store.label}.`);
    }
  }

  public async shutdown(): Promise<void> {
    await getDatabaseStore().shutdown();
  }

  private persist(dataToSave?: DatabaseSchema) {
    getDatabaseStore().save(dataToSave || this.data);
  }
  ...
  public verifyPassword(user: UserRecord, password: string): boolean {
    return verifyPasswordWithHash(password, user.passwordHash);
  }

  public createSession(userId: string): SessionRecord {
    const rawToken = generateSessionToken();
    const session: SessionRecord = {
      token: hashSessionToken(rawToken), // only the one-way hash is persisted
      userId,
      expiresAt: new Date(Date.now() + getSessionTtlMs()).toISOString(),
    };
    // Remove this user's prior sessions for cleanliness.
  ...
  public getSession(token: string): SessionRecord | undefined {
    ...
    const session = this.data.sessions.find((s) => s.token === tokenHash);
    if (!session) return undefined;
    if (new Date(session.expiresAt) < new Date()) {
      this.data.sessions = this.data.sessions.filter((s) => s.token !== tokenHash);
      this.save();
      return undefined;
    }
    return session;
  ...
  // --- Business Documents ---
  public getBusinessDocuments(workspaceId: string): BusinessDocumentRecord[] {
    return this.data.business_documents.filter((d) => d.workspaceId === workspaceId);
  }

  public getBusinessDocumentById(documentId: string, workspaceId: string): BusinessDocumentRecord | undefined {
    return this.data.business_documents.find((d) => d.id === documentId && d.workspaceId === workspaceId);
  }

  public createBusinessDocument(workspaceId: string, data: Partial<BusinessDocumentRecord> & { title: string; documentType: BusinessDocumentRecord['documentType'] }): BusinessDocumentRecord {
    const now = new Date().toISOString();
    const doc: BusinessDocumentRecord = {
    ...
    this.data.business_documents.push(doc);
    this.save();
    return doc;
  }

  public updateBusinessDocument(documentId: string, workspaceId: string, updates: Partial<BusinessDocumentRecord>): BusinessDocumentRecord {
    const doc = this.getBusinessDocumentById(documentId, workspaceId);
    if (!doc) throw new Error("Business document not found");
    Object.assign(doc, updates, { updatedAt: new Date().toISOString() });
    this.save();
    return doc;
  }

  public deleteBusinessDocument(documentId: string, workspaceId: string): boolean {
    const initialLen = this.data.business_documents.length;
    this.data.business_documents = this.data.business_documents.filter((d) => !(d.id === documentId && d.workspaceId === workspaceId));
    const deleted = this.data.business_documents.length < initialLen;
    if (deleted) {
      this.save();
    }
    return deleted;
  ...
  public getAllCreativeRequests(): CreativeRequestRecord[] {
    return [...(this.data.creative_requests || [])].sort(
      (a, b) => (b.createdAt || "").localeCompare(a.createdAt || "")
    );
  }

  public getCreativeRequestById(id: string): CreativeRequestRecord | undefined {
    return (this.data.creative_requests || []).find((r) => r.id === id);
  }

  public updateCreativeRequestById(id: string, updates: Partial<CreativeRequestRecord>): CreativeRequestRecord {
    const req = (this.data.creative_requests || []).find((r) => r.id === id);
    if (!req) throw new Error("Creative request not found");
    Object.assign(req, updates, { updatedAt: new Date().toISOString() });
    this.save();
    return req;
  }

  public addCreativeRequestRevision(
    requestId: string,
    revision: Omit<CreativeRequestRevisionRecord, "id" | "requestId" | "createdAt">
  ): CreativeRequestRevisionRecord {
    const req = (this.data.creative_requests || []).find((r) => r.id === requestId);
    if (!req) throw new Error("Creative request not found");
    if (!req.revisions) req.revisions = [];
    const created: CreativeRequestRevisionRecord = {
      id: "rev_" + crypto.randomUUID().substring(0, 8),
      requestId,
      createdAt: new Date().toISOString(),
      ...revision,
    };
    req.revisions.unshift(created);
    req.revisionCount = (req.revisionCount || 0) + 1;
    req.updatedAt = new Date().toISOString();
    this.save();
    return created;
  }

  public addCreativeRequestDelivery(
    requestId: string,
    delivery: Omit<CreativeRequestDeliveryRecord, "id" | "requestId" | "createdAt">
  ): CreativeRequestDeliveryRecord {
    const req = (this.data.creative_requests || []).find((r) => r.id === requestId);
    if (!req) throw new Error("Creative request not found");
    if (!req.deliveries) req.deliveries = [];
    const created: CreativeRequestDeliveryRecord = {
      id: "del_" + crypto.randomUUID().substring(0, 8),
      requestId,
      createdAt: new Date().toISOString(),
      ...delivery,
    };
    req.deliveries.push(created);
    req.deliverableIds = Array.from(new Set([...(req.deliverableIds || []), ...delivery.assetIds]));
    req.updatedAt = new Date().toISOString();
    this.save();
    return created;
  }

  public getProductionUsageOverview() {
    const requests = this.data.creative_requests || [];
    const countsByStatus: Record<string, number> = {};
    const countsByStudio: Record<string, number> = {};
    requests.forEach((r) => {
      const st = r.lifecycleStatus || "SUBMITTED";
      countsByStatus[st] = (countsByStatus[st] || 0) + 1;
      const stu = r.assignedStudio || "unassigned";
      countsByStudio[stu] = (countsByStudio[stu] || 0) + 1;
    });
    const revisions = requests.reduce((sum, r) => sum + (r.revisionCount || 0), 0);
    const deliveries = requests.reduce((sum, r) => sum + (r.deliveries?.length || 0), 0);
    const deliveredAssets = requests.reduce((sum, r) => sum + (r.deliverableIds?.length || 0), 0);
    return {
      totalRequests: requests.length,
      openRequests: requests.filter((r) => !["COMPLETED", "CANCELLED", "DELIVERED", "LIBRARY"].includes(r.lifecycleStatus || "")).length,
      deliveredAssets,
      deliveries,
      revisions,
      countsByStatus,
      countsByStudio,
      totalAssets: this.data.assets?.length || 0,
      workspacesInProduction: new Set(requests.map((r) => r.workspaceId)).size,
    };
  }
  ...
  public getAdminOverviewStats() {
    ...
    let dbSizeKb = 0;
    try {
      // Only report the JSON file size when the JSON store is actually active.
      // In Postgres mode this file is not the source of truth and must not be
      // read (or faked) for system-health reporting.
      if (getDatabaseStore().kind === "json" && fs.existsSync(DB_FILE)) {
  ...
  public getPlatformSettings(): PlatformSettingsRecord {
    const existing = this.data.platform_settings;
    if (existing) {
      // Fill configurable defaults when a legacy settings record lacks them
      // (stale JSON files / pre-Phase-3 rows). Writes persist them.
      if (!existing.planConfigs) existing.planConfigs = DEFAULT_PRODUCTION_PLAN_CONFIGS;
      if (!existing.productionConfig) existing.productionConfig = DEFAULT_PRODUCTION_CONFIG;
      return existing;
    }
    return {
      id: "global_settings",
      ...
      updatedAt: new Date().toISOString(),
      planConfigs: DEFAULT_PRODUCTION_PLAN_CONFIGS,
  ...
  // ==========================================
  // PRODUCTION JOB ENGINE (Production, Review & Delivery)
  // ==========================================

  public ensureProductionJobs(): void {
    if (!this.data.production_jobs) {
      this.data.production_jobs = [];
    }

    const requests = this.data.creative_requests || [];
    for (const req of requests) {
      const existing = this.data.production_jobs.find((j) => j.requestId === req.id || j.id === "pjob_" + req.id);
      if (!existing) {
        const user = this.getUserById(req.userId);
        const ws = this.getWorkspaceById(req.workspaceId);
        const proj = req.projectId ? this.getProjectById(req.projectId) : undefined;

        let status: ProductionJobStatus = "SUBMITTED";
        const lc = (req.lifecycleStatus || req.status || "").toUpperCase();
        if (lc === "COMPLETED" || lc === "DELIVERED") status = "DELIVERED";
        else if (lc === "APPROVED") status = "APPROVED";
        else if (lc === "REVISION_REQUESTED") status = "REVISION_REQUESTED";
        else if (lc === "CLIENT_REVIEW") status = "CLIENT_REVIEW";
        else if (lc === "INTERNAL_REVIEW") status = "INTERNAL_REVIEW";
        else if (lc === "IN_PRODUCTION" || lc === "IN-PRODUCTION") status = "IN_PRODUCTION";
        else if (lc === "ASSIGNED") status = "ASSIGNED";
        else if (lc === "TRIAGED") status = "TRIAGED";

        const pjobId = "pjob_" + req.id;
        const deliverables = buildDefaultDeliverables(req.serviceId, req.serviceName || req.title);
        deliverables.forEach((d) => {
          d.jobId = pjobId;
        });

        // If completed or in review, seed v1 version so user can immediately test review and approval!
        if (status === "DELIVERED" || status === "APPROVED" || status === "CLIENT_REVIEW" || status === "REVISION_REQUESTED") {
          deliverables.forEach((d, idx) => {
            const vNum = 1;
            d.currentVersionNumber = vNum;
            d.status = status === "DELIVERED" ? "delivered" : status === "APPROVED" ? "approved" : status === "REVISION_REQUESTED" ? "revision_requested" : "client_review";
            const sampleUrl = d.category === "video" || d.category === "motion"
              ? "https://assets.mixkit.co/videos/preview/mixkit-abstract-flowing-neon-lights-31802-large.mp4"
              : d.category === "document" || d.category === "epk"
              ? "https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf"
              : "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=1200&auto=format&fit=crop&q=80";

            d.versions.push({
              id: "dver_" + crypto.randomUUID().substring(0, 8),
              deliverableId: d.id,
              jobId: pjobId,
              versionNumber: vNum,
              title: `${d.title} (v${vNum})`,
              previewUrl: sampleUrl,
              fileUrl: sampleUrl,
              fileSize: "14.2 MB",
              fileType: d.category === "video" ? "video/mp4" : d.category === "document" ? "application/pdf" : "image/png",
              notes: idx === 0 ? "Initial master delivery prepared for client review." : "Supporting distribution asset bundle.",
              uploadedBy: req.assignedProducer || "Dare Balogun",
              uploadedAt: req.createdAt || new Date().toISOString(),
              status: d.status,
              comments: [
                {
                  id: "comm_" + crypto.randomUUID().substring(0, 8),
                  authorId: "usr_studio_lead",
                  authorName: req.assignedProducer || "Dare Balogun",
                  authorRole: "producer",
                  content: "V1 is uploaded and calibrated to the DSP master standards. Looking forward to your thoughts!",
        ...
        const newJob: ProductionJobRecord = {
          ...
          identity: (ws?.identityType as any) || (req.serviceId.startsWith("brand") ? "brand" : "artist"),
          projectId: req.projectId,
          projectTitle: proj?.title,
          serviceId: req.serviceId,
          serviceName: req.serviceName || req.title || "Studio Service",
          deliverables,
          assignedStudio: req.assignedStudio || "artwork",
          assignedProducer: req.assignedProducer || "Dare Balogun",
          assignedAdminId: req.assignedAdminId,
          priority: req.priority || "medium",
          dueDate: req.deadline || "2026-11-01",
          status,
          internalNotes: req.internalNotes || "Internal Studio Check: Initial brief calibrated.",
          clientVisibleNotes: req.clientVisibleNotes || "Our creative production team is actively working on your deliverables.",
          revisions: [],
          approvals: [],
          createdAt: req.createdAt || new Date().toISOString(),
        ...
        if (status === "DELIVERED") {
          newJob.finalDelivery = {
            deliveredAt: new Date().toISOString(),
            deliveredBy: "system_admin",
            deliveredByName: "Keedohub Studio Production",
            note: "Production completed and final assets dispatched to your Project and Library.",
            createdAssetIds: [],
          };
        }

        this.data.production_jobs.push(newJob);
      }
    }
  }

  public getProductionJobs(workspaceId?: string, projectId?: string): ProductionJobRecord[] {
    this.ensureProductionJobs();
    let jobs = this.data.production_jobs || [];
    if (workspaceId) {
  ...
  public getProductionJobById(id: string): ProductionJobRecord | undefined {
    this.ensureProductionJobs();
    return (this.data.production_jobs || []).find((j) => j.id === id || j.requestId === id);
  }

  public getProductionJobByRequestId(requestId: string): ProductionJobRecord | undefined {
    this.ensureProductionJobs();
    return (this.data.production_jobs || []).find((j) => j.requestId === requestId);
  }

  public createProductionJob(
    workspaceId: string,
    data: Omit<ProductionJobRecord, "id" | "workspaceId" | "createdAt" | "updatedAt">
  ): ProductionJobRecord {
    this.ensureProductionJobs();
    const id = "pjob_" + crypto.randomUUID().substring(0, 8);
    const now = new Date().toISOString();

    const deliverables = (data.deliverables && data.deliverables.length > 0)
      ? data.deliverables
      : buildDefaultDeliverables(data.serviceId, data.serviceName);
    ...
    this.data.production_jobs.unshift(newJob);
    this.save();
    return newJob;
  }

  public updateProductionJob(id: string, updates: Partial<ProductionJobRecord>): ProductionJobRecord {
    this.ensureProductionJobs();
    const job = this.getProductionJobById(id);
    if (!job) throw new Error("Production job not found: " + id);

    Object.assign(job, updates, { updatedAt: new Date().toISOString() });

    // Sync back to corresponding creative_request if present
    if (job.requestId) {
      const req = this.getCreativeRequestById(job.requestId);
      if (req) {
        if (updates.status) {
          req.lifecycleStatus = updates.status;
          if (updates.status === "DELIVERED") req.status = "completed";
          else if (updates.status === "IN_PRODUCTION") req.status = "in-production";
        }
        if (updates.assignedStudio) req.assignedStudio = updates.assignedStudio as any;
        if (updates.assignedProducer) req.assignedProducer = updates.assignedProducer;
        if (updates.priority) req.priority = updates.priority;
        if (updates.dueDate) req.deadline = updates.dueDate;
        if (updates.internalNotes !== undefined) req.internalNotes = updates.internalNotes;
        if (updates.clientVisibleNotes !== undefined) req.clientVisibleNotes = updates.clientVisibleNotes;
        req.updatedAt = new Date().toISOString();
      }
    }
  ...
  public addDeliverableVersion(
    ...
    versionData: {
      ...
      notes?: string;
      uploadedBy: string;
      status?: "internal_review" | "client_review" | "revision_requested" | "approved" | "delivered";
    }
  ): { job: ProductionJobRecord; version: ProductionDeliverableVersionRecord } {
    this.ensureProductionJobs();
    const job = this.getProductionJobById(jobId);
    if (!job) throw new Error("Production job not found: " + jobId);

    const deliverable = job.deliverables.find((d) => d.id === deliverableId);
    if (!deliverable) throw new Error("Deliverable not found: " + deliverableId);

    const versionNumber = (deliverable.currentVersionNumber || 0) + 1;
    const versionStatus = versionData.status || "client_review";
    const now = new Date().toISOString();

    const newVersion: ProductionDeliverableVersionRecord = {
      id: "dver_" + crypto.randomUUID().substring(0, 8),
      deliverableId,
      jobId,
      versionNumber,
      title: versionData.title || `${deliverable.title} (v${versionNumber})`,
      previewUrl: versionData.previewUrl,
      fileUrl: versionData.fileUrl || versionData.previewUrl,
      fileSize: versionData.fileSize || "12 MB",
      fileType: versionData.fileType || "image/png",
    ...
    deliverable.currentVersionNumber = versionNumber;
    deliverable.status = versionStatus;

    if (versionStatus === "client_review") {
      job.status = "CLIENT_REVIEW";
      if (job.requestId) {
        const req = this.getCreativeRequestById(job.requestId);
        if (req) req.lifecycleStatus = "CLIENT_REVIEW";
      }
    }

    job.updatedAt = now;
    this.save();
    return { job, version: newVersion };
  }

  public addJobRevision(
    jobId: string,
    revisionData: {
      deliverableId?: string;
      deliverableTitle?: string;
  ...
  ): { job: ProductionJobRecord; revision: ProductionRevisionRecord } {
    this.ensureProductionJobs();
    const job = this.getProductionJobById(jobId);
    if (!job) throw new Error("Production job not found: " + jobId);

    const now = new Date().toISOString();
    const rev: ProductionRevisionRecord = {
      id: "prev_" + crypto.randomUUID().substring(0, 8),
      jobId,
      deliverableId: revisionData.deliverableId,
      deliverableTitle: revisionData.deliverableTitle,
      versionNumber: revisionData.versionNumber,
      reason: revisionData.reason,
      requestedChanges: revisionData.requestedChanges,
      referenceUrl: revisionData.referenceUrl,
      requestedBy: revisionData.requestedBy,
      requesterId: revisionData.requesterId,
      status: "OPEN",
      createdAt: now,
    };

    job.revisions = job.revisions || [];
    job.revisions.unshift(rev);
    job.status = "REVISION_REQUESTED";
    job.updatedAt = now;

    if (revisionData.deliverableId) {
      const del = job.deliverables.find((d) => d.id === revisionData.deliverableId);
      if (del) {
        del.status = "revision_requested";
    ...
    // Sync back to creative request
    if (job.requestId) {
      const req = this.getCreativeRequestById(job.requestId);
      if (req) {
        req.lifecycleStatus = "REVISION_REQUESTED";
        req.revisionCount = (req.revisionCount || 0) + 1;
        req.updatedAt = now;
      }
    }
    ...
    this.save();
    return { job, revision: rev };
  }

  public addJobApproval(
    jobId: string,
    approvalData: {
      deliverableId?: string;
      versionNumber?: number;
  ...
  ): { job: ProductionJobRecord; approval: ProductionApprovalRecord } {
    this.ensureProductionJobs();
    const job = this.getProductionJobById(jobId);
    if (!job) throw new Error("Production job not found: " + jobId);

    const now = new Date().toISOString();
    const apprv: ProductionApprovalRecord = {
      id: "papp_" + crypto.randomUUID().substring(0, 8),
      jobId,
      deliverableId: approvalData.deliverableId,
      versionNumber: approvalData.versionNumber,
    ...
    job.approvals.unshift(apprv);

    if (approvalData.deliverableId) {
      const del = job.deliverables.find((d) => d.id === approvalData.deliverableId);
      if (del) {
        del.status = "approved";
        const latest = del.versions[del.versions.length - 1];
      ...
      // Mark all deliverables approved
    ...
    const allApproved = job.deliverables.every((d) => d.status === "approved" || d.versions.length === 0);
    if (allApproved || !approvalData.deliverableId) {
      job.status = "APPROVED";
      if (job.requestId) {
        const req = this.getCreativeRequestById(job.requestId);
        if (req) req.lifecycleStatus = "APPROVED";
      }
    }

    job.updatedAt = now;
    this.save();
    return { job, approval: apprv };
  }

  public addDeliverableComment(
    jobId: string,
    deliverableId: string,
    versionNumber: number,
    commentData: {
    ...
    const version = deliverable.versions.find((v) => v.versionNumber === versionNumber) || deliverable.versions[deliverable.versions.length - 1];
    if (!version) throw new Error("Version not found on deliverable");

    const comment: DeliverableCommentRecord = {
      id: "comm_" + crypto.randomUUID().substring(0, 8),
  ...
  public finalizeJobDelivery(
  ...
  ): { job: ProductionJobRecord; createdAssets: AssetRecord[] } {
    this.ensureProductionJobs();
    const job = this.getProductionJobById(jobId);
    if (!job) throw new Error("Production job not found: " + jobId);
    ...
    // Create real assets in Workspace Project & Library for each deliverable with a version
    for (const del of job.deliverables) {
      del.status = "delivered";
      const latestVer = del.versions[del.versions.length - 1];
      if (latestVer) {
        latestVer.status = "delivered";
        const asset = this.createAsset(job.workspaceId, {
          name: del.title,
          category: (del.category as any) || "artwork",
          url: latestVer.fileUrl || latestVer.previewUrl,
          mimeType: latestVer.fileType || (del.category === "video" ? "video/mp4" : del.category === "document" ? "application/pdf" : "image/png"),
          size: 3200000,
          folderName: "Deliveries",
          projectId: job.projectId,
          tags: ["delivery", "studio", job.serviceId, `v${latestVer.versionNumber}`],
          metadata: {
            approved: true,
            deliveryFromJobId: job.id,
    ...
    job.finalDelivery = {
      ...
      deliveredByName: deliveryData.deliveredByName,
      note: deliveryData.note || "Final production assets approved and dispatched to Project & Library.",
      createdAssetIds,
    };
    job.updatedAt = now;

    // Sync back to creative_request
    if (job.requestId) {
      const req = this.getCreativeRequestById(job.requestId);
      if (req) {
        req.lifecycleStatus = "DELIVERED";
        req.status = "completed";
        req.deliverableIds = createdAssetIds;
        req.updatedAt = now;
      }
    }

    this.save();
    return { job, createdAssets };
  }
}

function buildDefaultDeliverables(serviceId: string, serviceName?: string): ProductionDeliverableRecord[] {
  const norm = (serviceId || "").toLowerCase();
  if (norm.includes("cover") || norm.includes("artwork")) {
    return [
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "3000×3000px 300DPI Master Artwork",
        category: "cover",
        format: "PNG / JPG",
        specs: "3000×3000px RGB 300DPI DSP-compliant",
      ...
      },
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "Spotify Canvas 9:16 Video Loop",
        category: "video",
        format: "MP4",
        specs: "1080×1920px 60fps 5-8s seamless loop",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
      },
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "Tracklist Back Cover Layout",
        category: "artwork",
        format: "PNG / PDF",
        specs: "3000×3000px 300DPI",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
      },
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "Social Promo Thumbnail Suite",
        category: "social",
        format: "ZIP",
        specs: "1:1, 4:5, 16:9 crops",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
  ...
  if (norm.includes("visual") || norm.includes("motion")) {
    return [
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "16:9 Full HD Kinetic Visualizer Video",
        category: "video",
        format: "MP4",
        specs: "1920×1080 60fps Full Stream",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
      },
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "9:16 Vertical Reel & Story Cut",
        category: "video",
        format: "MP4",
        specs: "1080×1920 60fps 30s Hook",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
      },
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "Seamless Concert Stage Backdrop Stems",
        category: "motion",
        format: "ProRes / MOV",
        specs: "Seamless LED wall projection loop",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
  ...
  if (norm.includes("brand_identity") || norm.includes("brand-identity") || norm.includes("brand_kit")) {
    return [
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "Primary, Secondary & Icon Vector Marks",
        category: "brand",
        format: "SVG / EPS / PNG",
        specs: "Full scalable vector logo suite in light, dark & mono",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
      },
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "Color Palette Tokens & Contrast Matrix",
        category: "brand",
        format: "JSON / PDF",
        specs: "Hex, RGB, CMYK & WCAG AA contrast matrix",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
      },
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "Typography Pairing Hierarchy System",
        category: "document",
        format: "PDF / Web Fonts",
        specs: "Display, heading and body typeface hierarchy",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
      },
      {
        ...
        category: "document",
        format: "PDF",
        specs: "24-page executive brand bible",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
  ...
  if (norm.includes("epk") || norm.includes("press")) {
    return [
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "Interactive Press Kit Deck",
        category: "epk",
        format: "Interactive PDF",
        specs: "12-page interactive PDF with audio/DSP links",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
      },
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "Print-Ready Press One-Sheet",
        category: "document",
        format: "PDF (CMYK)",
        specs: "A4 / Letter 300DPI with print bleeds",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
      },
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "Curator & Editorial Pitch Deck",
        category: "document",
        format: "PDF",
        specs: "DSP playlist & media pitch presentation",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
  ...
  if (norm.includes("presentation") || norm.includes("deck")) {
    return [
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "20-Slide Master Keynote/Pitch Deck",
        category: "document",
        format: "PDF / PPTX",
        specs: "16:9 Widescreen investor-grade presentation deck",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
      },
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "Custom Visual Infographics & Charts",
        category: "artwork",
        format: "SVG / PNG",
        specs: "Vector market diagrams & stat visualizers",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
      },
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "Editable Slide Template System",
        category: "document",
        format: "Figma / PPTX",
        specs: "Modular layout library for ongoing updates",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
  ...
  if (norm.includes("document") || norm.includes("stationery")) {
    return [
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "Print-Ready Letterhead & Invoice PDFs",
        category: "document",
        format: "PDF (CMYK)",
        specs: "A4 & Letter 300DPI print bleed vectors",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
      },
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "Editable Business Proposal Template",
        category: "document",
        format: "DOCX / PDF",
        specs: "Formal multi-page proposal template",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
      },
      {
        id: "del_" + crypto.randomUUID().substring(0, 8),
        jobId: "",
        title: "Official 8-Page Company Profile Document",
        category: "document",
        format: "PDF",
        specs: "Executive corporate capabilities brochure",
        downloadAllowed: true,
        status: "pending",
        currentVersionNumber: 0,
        versions: [],
  ...
  return [
    {
      id: "del_" + crypto.randomUUID().substring(0, 8),
      jobId: "",
      title: `${serviceName || "Production Master"} Primary Asset`,
      category: "artwork",
      format: "High-Resolution Master",
      specs: "Production-grade deliverable asset",
      downloadAllowed: true,
      status: "pending",
      currentVersionNumber: 0,
      versions: [],
    },
    {
      id: "del_" + crypto.randomUUID().substring(0, 8),
      jobId: "",
      title: `${serviceName || "Production"} Social & Digital Cuts`,
      category: "social",
      format: "PNG / MP4",
      specs: "Multi-channel distribution formats",
      downloadAllowed: true,
      status: "pending",
      currentVersionNumber: 0,
      versions: [],
