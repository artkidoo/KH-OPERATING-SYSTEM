...
apiRouter.post("/auth/demo", async (req: Request, res: Response) => {
  ...
  if (demoType !== "artist" && demoType !== "brand" && demoType !== "admin") {
    return res.status(400).json({ error: "Invalid demo type. Use 'artist', 'brand', or 'admin'." });
  }

  try {
    const isArtist = demoType === "artist";
    const isAdmin = demoType === "admin";
    const demoName = isArtist ? "Demo Artist" : isAdmin ? "Platform Lead Admin" : "Demo Brand";
    const demoEmail = isArtist 
      ? `demo.artist.${Date.now()}@keedohub.demo` 
      : isAdmin
    ...
    const demoWorkspace = db.createWorkspace(
      ...
      isArtist 
        ? "Independent artist pushing creative boundaries."
        : "Modern brand streamlining creative production.",
      isArtist ? "Afro-Fusion" : "Technology",
      "",
    );
    ...
    // Seed demo data
    if (isArtist) {
      const project = db.createProject(demoWorkspace.id, {
        title: "Summer Album 2026",
        description: "New Afrobeats single with full creative package",
        category: "music",
        status: "in-progress",
        priority: "high",
        budget: 0,
      ...
      db.createRelease(demoWorkspace.id, {
        ...
        status: "ready",
        narrative: "A late-night drive through Lagos streets.",
        phases: [],
        checklist: [],
      });
      
      db.createAsset(demoWorkspace.id, {
        ...
        mimeType: "image/jpeg",
        tags: ["cover", "artwork"],
        metadata: { approved: true, favorite: true },
      });
      ...
      db.createCreativeRequest(demoWorkspace.id, {
        ...
        currency: "USD",
        deadline: "Flexible",
        briefDetails: "Professional cover artwork for single release",
        status: "completed",
        title: "Album Cover Request",
        requestType: "artwork",
        lifecycleStatus: "COMPLETED",
        membershipEntitlement: "artist_pro",
        priority: "medium",
      });
      ...
      db.createProductionJob(demoWorkspace.id, {
        ...
        serviceId: "cover-art",
        serviceName: "Master Cover Artwork & Single Packaging",
        deliverables: [
          {
            id: `del_artist_${Date.now()}`,
            jobId: "",
            title: "Master Cover Artwork 3000x3000",
            category: "artwork",
            format: "JPG / PNG",
            specs: "3000x3000px 300DPI RGB Color",
            ...
            versions: [
              {
                id: `ver_art_1`,
                deliverableId: `del_artist_demo`,
                jobId: `pjob_demo_artist`,
                versionNumber: 1,
                title: "Front Sleeve Concept v1",
                ...
                status: "client_review",
                notes: "Visual Studio lead initial high-res deliverable for client review.",
                uploadedBy: "Visual Studio Lead",
                uploadedAt: new Date().toISOString(),
              },
            ],
          },
        ],
        assignedStudio: "Visual Art & Artwork",
        assignedProducer: "Dare Balogun",
        priority: "high",
        dueDate: "2026-09-25",
        status: "CLIENT_REVIEW",
        internalNotes: "Internal QA passed. Composition and color fidelity verified.",
        clientVisibleNotes: "Version 1 sleeve concept is now ready for your review. Please inspect lighting, title typography, and let us know if revisions are required.",
        revisions: [],
        approvals: [],
      });
    } else {
      const project = db.createProject(demoWorkspace.id, {
        ...
        description: "Complete brand identity update",
        category: "brand",
        status: "in-progress",
        priority: "high",
        budget: 0,
      ...
      db.createBrandDNA(demoWorkspace.id, {
        identity: "Demo Brand Co.",
        positioning: "Premium design solutions for forward-thinking companies",
        businessCategory: "Technology",
        audience: "Young professionals, 25-40",
        valueProposition: "Building the future, beautifully.",
        contentPillars: ["Design", "Technology", "Innovation"],
        growthGoals: ["Expand into new markets", "Grow client base"],
      });
      
      db.createBusinessDocument(demoWorkspace.id, {
        documentType: "company_profile",
        status: "draft",
        title: "Demo Brand Co. - Company Profile",
        content: { html: "<p>Welcome to Demo Brand Co.</p>" },
      });
      ...
      db.createCreativeRequest(demoWorkspace.id, {
        ...
        currency: "USD",
        deadline: "2026-11-01",
        briefDetails: "Complete brand identity package",
        status: "completed",
        title: "Brand Identity Package",
        requestType: "brand asset",
        lifecycleStatus: "COMPLETED",
        membershipEntitlement: "brand_pro",
        priority: "high",
      });
      ...
      db.createProductionJob(demoWorkspace.id, {
        ...
        projectTitle: project.title,
        serviceId: "brand_identity",
        serviceName: "Brand Identity System & Vector Suite",
        deliverables: [
          {
            id: `del_brand_${Date.now()}`,
            jobId: "",
            title: "Brand Guidelines & Vector Suite",
            category: "brand",
            format: "PDF / Vector",
            specs: "Master vector kit and guidelines",
            downloadAllowed: true,
            status: "client_review",
            currentVersionNumber: 1,
            versions: [
              {
                id: `ver_brand_1`,
                deliverableId: `del_brand_demo`,
                jobId: `pjob_demo_brand`,
                versionNumber: 1,
                title: "Identity Guide v1",
                previewUrl: "https://images.unsplash.com/photo-1561070791-2526d30994b5?w=1200",
                fileUrl: "https://images.unsplash.com/photo-1561070791-2526d30994b5?w=1200",
                status: "client_review",
                notes: "Complete vector logomark, typographic hierarchy and color system.",
                uploadedBy: "Brand Studio Lead",
                uploadedAt: new Date().toISOString(),
              },
            ],
          },
        ],
        assignedStudio: "Brand Identity",
        assignedProducer: "Dare Balogun",
        priority: "high",
        dueDate: "2026-11-15",
        status: "CLIENT_REVIEW",
        internalNotes: "Vector guidelines generated, contrast ratios WCAG AAA checked.",
        clientVisibleNotes: "Brand guidelines v1 is ready for executive review. You may approve or submit specific revision feedback directly.",
        revisions: [],
        approvals: [],
      });
    }
...
apiRouter.get("/workspaces/:workspaceId/business-documents", requireAuth, requireWorkspaceAccess, (req: AuthenticatedRequest, res: Response) => {
  const documents = db.getBusinessDocuments(req.params.workspaceId);
  res.json({ documents });
});

apiRouter.get("/workspaces/:workspaceId/business-documents/:documentId", requireAuth, requireWorkspaceAccess, (req: AuthenticatedRequest, res: Response) => {
  const document = db.getBusinessDocumentById(req.params.documentId, req.params.workspaceId);
  if (!document) return res.status(404).json({ error: "Document not found" });
  res.json({ document });
});

apiRouter.post("/workspaces/:workspaceId/business-documents", requireAuth, requireWorkspaceAccess, (req: AuthenticatedRequest, res: Response) => {
  const { documentType, title, status, recipientName, recipientCompany, recipientEmail, content } = req.body;
  if (!documentType || !title) {
    return res.status(400).json({ error: "Document type and title are required" });
  }
  try {
    const doc = db.createBusinessDocument(req.params.workspaceId, {
      documentType,
      title,
      status,
      recipientName,
...
apiRouter.put("/workspaces/:workspaceId/business-documents/:documentId", requireAuth, requireWorkspaceAccess, (req: AuthenticatedRequest, res: Response) => {
  try {
    const updated = db.updateBusinessDocument(req.params.documentId, req.params.workspaceId, req.body);
...
apiRouter.delete("/workspaces/:workspaceId/business-documents/:documentId", requireAuth, requireWorkspaceAccess, (req: AuthenticatedRequest, res: Response) => {
  const success = db.deleteBusinessDocument(req.params.documentId, req.params.workspaceId);
...
apiRouter.get("/workspaces/:workspaceId/creative-requests", requireAuth, requireWorkspaceAccess, (req: AuthenticatedRequest, res: Response) => {
  ...
  // Security: internal production fields are never sent to customers.
  const isAdmin = ["super_admin", "admin", "support"].includes((req.user!.systemRole as string) || "user");
  const sanitized = requests.map((r: any) => {
    const copy: any = { ...r };
    if (!isAdmin) {
      delete copy.internalNotes;
      delete copy.assignedAdminId;
      delete copy.assignedProducer;
      delete copy.productionStage;
      // Customers see their own revision requests but never internal resolution data.
      if (copy.revisions?.length) {
        copy.revisions = copy.revisions.map((rv: any) => ({
      ...
      if (copy.approvals) {
        copy.approvals = copy.approvals.map((a: any) => ({ by: a.by, at: a.at, note: a.note }));
      }
    }
    return copy;
  });
...
apiRouter.post("/workspaces/:workspaceId/creative-requests", requireAuth, requireWorkspaceAccess, (req: AuthenticatedRequest, res: Response) => {
  const { serviceId, serviceName, budget, currency, deadline, briefDetails, title, description, requestType, projectId, instructions, references, assetIds, priority, membershipEntitlement, lifecycleStatus, assignedStudio } = req.body;
  const label = serviceName || title || requestType;
  const details = briefDetails || description;
  if (!label || !details) {
  ...
  const creativeRequest = db.createCreativeRequest(req.params.workspaceId, {
    ...
    requestType: requestType || serviceName || "custom",
    projectId,
    instructions,
    references,
    assetIds,
    ...
    lifecycleStatus: lifecycleStatus || "SUBMITTED",
    assignedStudio,
    revisionCount: 0,
    approvals: [],
    deliverableIds: [],
  ...
  // Sync to canonical Production Job engine
  const ws = db.getWorkspaceById(req.params.workspaceId);
  const proj = projectId ? db.getProjects(req.params.workspaceId).find((p) => p.id === projectId) : undefined;
  db.createProductionJob(req.params.workspaceId, {
    requestId: creativeRequest.id,
    userId: req.user!.id,
    customerName: req.user!.fullName || "Customer",
    customerEmail: req.user!.email,
    identity: (ws?.identityType as any) || (serviceId?.startsWith("brand") ? "brand" : "artist"),
    projectId,
    projectTitle: proj?.title,
    serviceId: serviceId || "custom-studio-request",
    serviceName: label,
    deliverables: [],
    assignedStudio: (assignedStudio as any) || "artwork",
    assignedProducer: "Dare Balogun",
    priority: priority || "medium",
    dueDate: deadline || "Flexible",
    status: "SUBMITTED",
    internalNotes: "New studio request submitted. Pending triage.",
    clientVisibleNotes: "Your request has been received and added to the studio production queue.",
    revisions: [],
    approvals: [],
  });
  ...
  try {
    db.addNotification(
      ...
      `Your request for '${label}' was received and queued for studio production.`,
      "info",
      projectId ? `/projects?id=${projectId}` : "/requests"
    );
  } catch (_err) {
    // Non-fatal
  }
...
// Phase 1: request lifecycle update (customer visible status + admin production routing).
apiRouter.put("/workspaces/:workspaceId/creative-requests/:requestId", requireAuth, requireWorkspaceAccess, (req: AuthenticatedRequest, res: Response) => {
  const list = db.getCreativeRequests(req.params.workspaceId);
  const found: any = list.find((r: any) => r.id === req.params.requestId);
  if (!found) return res.status(404).json({ error: "Request not found" });
  const isAdmin = ["super_admin", "admin", "support"].includes((req.user!.systemRole as string) || "user");
  if (isAdmin) {
    Object.assign(found, req.body, { updatedAt: new Date().toISOString() });
  } else {
    // Customers may only adjust non-lifecycle brief fields. Status moves are
    // handled exclusively through the revision endpoint (guarded) and admin.
    const whitelist = ["priority", "instructions", "references", "requestType", "title", "description"];
...
// Customer revision request — guarded by plan revision allowance, no infinite loops.
apiRouter.post("/workspaces/:workspaceId/creative-requests/:requestId/revision", requireAuth, requireWorkspaceAccess, (req: AuthenticatedRequest, res: Response) => {
  const { reason, instructions } = req.body || {};
  if (!reason || !reason.trim()) {
    return res.status(400).json({ error: "A short reason for the revision is required" });
  }
  const found: any = db.getCreativeRequests(req.params.workspaceId).find((r: any) => r.id === req.params.requestId);
  if (!found) return res.status(404).json({ error: "Request not found" });
  const stage = found.lifecycleStatus || "SUBMITTED";
  if (!["IN_REVIEW", "CLIENT_REVIEW", "INTERNAL_QC", "FINAL_APPROVAL"].includes(stage)) {
    return res.status(409).json({ error: "This request is not in a review state yet. Revisions open when work moves into client review." });
  }
  const plan = (found.membershipEntitlement as string) || "artist_free";
  const revLimit = (plan.endsWith("_pro") || plan === "agency" || plan === "label" || plan === "enterprise") ? 5 : 2;
  const used = found.revisions?.length || 0;
  if (used >= revLimit) {
    return res.status(429).json({ error: `Revision limit reached for your current membership (${used}/${revLimit}). Upgrade to Pro for a higher allowance.` });
  }
  const revision = db.addCreativeRequestRevision(found.id, {
    revisionNumber: used + 1,
    reason: reason.trim(),
    instructions: (instructions || "").trim(),
    previousDeliverableId: found.deliverableIds?.[found.deliverableIds.length - 1],
    status: "OPEN",
    requestedBy: "customer",
  });
  db.updateCreativeRequestById(found.id, { lifecycleStatus: "REVISION_REQUESTED" });

  // Sync to production job
  const job = db.getProductionJobByRequestId(found.id);
  if (job) {
    db.addJobRevision(job.id, {
      reason: reason.trim(),
      requestedChanges: (instructions || reason).trim(),
      requestedBy: req.user!.fullName || "Customer",
  ...
  db.logActivity(
    ...
    "creative_request",
    found.id,
    `Customer requested revision #${used + 1} on '${found.title || found.serviceName}'`
  );
  res.status(201).json({ revision, request: db.getCreativeRequestById(found.id) });
});

// ==========================================
// PRODUCTION JOB ENGINE (Customer & Project Review Room)
// ==========================================

apiRouter.get("/workspaces/:workspaceId/production-jobs", requireAuth, requireWorkspaceAccess, (req: AuthenticatedRequest, res: Response) => {
  const { projectId } = req.query;
  const jobs = db.getProductionJobs(req.params.workspaceId, projectId as string | undefined);
  const isAdmin = ["super_admin", "admin", "support"].includes((req.user!.systemRole as string) || "user");
  ...
  const sanitized = jobs.map((job) => {
    ...
    if (!isAdmin) {
      delete copy.internalNotes;
      delete copy.assignedAdminId;
      // Filter out internal-only versions from deliverables for clients
      if (copy.deliverables) {
        copy.deliverables = copy.deliverables.map((del: any) => ({
          ...del,
          versions: (del.versions || []).filter((v: any) => v.status !== "internal"),
        }));
      }
    }
    return copy;
...
apiRouter.get("/workspaces/:workspaceId/production-jobs/:jobId", requireAuth, requireWorkspaceAccess, (req: AuthenticatedRequest, res: Response) => {
  const job = db.getProductionJobById(req.params.jobId);
  if (!job || job.workspaceId !== req.params.workspaceId) {
...
apiRouter.post(["/workspaces/:workspaceId/production-jobs/:jobId/revision", "/workspaces/:workspaceId/production-jobs/:jobId/revisions"], requireAuth, requireWorkspaceAccess, (req: AuthenticatedRequest, res: Response) => {
  const { deliverableId, deliverableTitle, versionNumber, reason, requestedChanges, referenceUrl } = req.body || {};
  if (!reason || !reason.trim()) {
    return res.status(400).json({ error: "A clear reason for the revision is required" });
  }
  ...
  const result = db.addJobRevision(job.id, {
    deliverableId,
    deliverableTitle,
    versionNumber: Number(versionNumber) || undefined,
    reason: reason.trim(),
    requestedChanges: (requestedChanges || reason).trim(),
    referenceUrl,
    requestedBy: req.user!.fullName || "Client",
    requesterId: req.user!.id,
  });
  ...
  db.logActivity(
    ...
    "production_job",
    job.id,
    `Revision requested on '${job.serviceName}': ${reason}`
  );
  ...
  try {
    db.addNotification(
      req.params.workspaceId,
      "Revision Requested",
      `Your revision request for '${job.serviceName}' has been submitted and queued with the production team.`,
      "info",
      job.projectId ? `/projects?id=${job.projectId}` : "/requests"
    );
  } catch (_err) {
    // Non-fatal
  }

  res.status(201).json({ success: true, job: result.job, revision: result.revision });
});

apiRouter.post("/workspaces/:workspaceId/production-jobs/:jobId/approve", requireAuth, requireWorkspaceAccess, (req: AuthenticatedRequest, res: Response) => {
  const { deliverableId, versionNumber, note, finalizeNow } = req.body || {};
  const job = db.getProductionJobById(req.params.jobId);
  if (!job || job.workspaceId !== req.params.workspaceId) {
    return res.status(404).json({ error: "Production job not found" });
  }

  const result = db.addJobApproval(job.id, {
    deliverableId,
    versionNumber: Number(versionNumber) || undefined,
    approvedBy: req.user!.id,
    approvedByName: req.user!.fullName || "Client",
    note,
  });

  let createdAssets: any[] = [];
  // If finalizeNow is requested or if all deliverables are approved
  if (finalizeNow || result.job.status === "APPROVED") {
    const finalResult = db.finalizeJobDelivery(job.id, {
      deliveredBy: req.user!.id,
      deliveredByName: req.user!.fullName || "Client Approved",
      note: note || "Client approved deliverables. Assets created and dispatched to Project & Library.",
    });
    createdAssets = finalResult.createdAssets;
    result.job = finalResult.job;
  }
  ...
  db.logActivity(
    ...
    "production_job",
    job.id,
    `Approved deliverable on '${job.serviceName}'`
  );
  ...
  try {
    db.addNotification(
      ...
      `You approved '${job.serviceName}'. Deliverables have been finalized and placed in your Project and Library.`,
      "success",
      job.projectId ? `/projects?id=${job.projectId}` : "/library"
    );
  } catch (_err) {
    // Non-fatal
  }

  res.json({ success: true, job: result.job, createdAssets });
});

apiRouter.post("/workspaces/:workspaceId/production-jobs/:jobId/comments", requireAuth, requireWorkspaceAccess, (req: AuthenticatedRequest, res: Response) => {
  const { deliverableId, versionNumber, content } = req.body || {};
  if (!deliverableId || !content || !content.trim()) {
    return res.status(400).json({ error: "deliverableId and comment content are required" });
  }
  ...
  const isAdmin = ["super_admin", "admin", "support"].includes((req.user!.systemRole as string) || "user");
  const comment = db.addDeliverableComment(job.id, deliverableId, Number(versionNumber) || 1, {
...
apiRouter.post("/workspaces/:workspaceId/production-jobs/:jobId/finalize-delivery", requireAuth, requireWorkspaceAccess, (req: AuthenticatedRequest, res: Response) => {
  const job = db.getProductionJobById(req.params.jobId);
  if (!job || job.workspaceId !== req.params.workspaceId) {
    return res.status(404).json({ error: "Production job not found" });
  }
  ...
  const { note } = req.body || {};
  const { job: updatedJob, createdAssets } = db.finalizeJobDelivery(job.id, {
    deliveredBy: req.user!.id,
    deliveredByName: req.user!.fullName || "Delivery Manager",
    note,
  });
  ...
  db.logActivity(
    ...
    "production_job",
    job.id,
    `Finalized delivery for '${job.serviceName}' (${createdAssets.length} assets created)`
  );
  ...
  try {
    db.addNotification(
      ...
      `Final assets for '${job.serviceName}' are now ready in your Project and Library.`,
      "success",
      job.projectId ? `/projects?id=${job.projectId}` : "/library"
    );
  } catch (_err) {
  ...
  res.json({ success: true, job: updatedJob, createdAssets });
});

// ==========================================
// PHASE 3 — ADMIN PRODUCTION CENTER (admin-only, cross-workspace)
// ==========================================

const PRODUCTION_NEXT: Record<string, string[]> = {
...
function enrichProductionRequest(r: any) {
  const ws = db.getWorkspaceById(r.workspaceId);
  const user = r.userId ? db.getUserById(r.userId) : undefined;
  const proj = r.projectId ? db.getProjects(r.workspaceId).find((p) => p.id === r.projectId) : undefined;
  return {
    ...r,
    customerName: user?.fullName || r.requesterId || "Customer",
    workspaceName: ws?.name || "Workspace",
    workspaceIdentity: ws?.identityType || "artist",
...
apiRouter.get("/admin/production/requests", requireAuth, requireAdmin(["super_admin", "admin"]), (req: AuthenticatedRequest, res: Response) => {
  ...
  if (q.status) requests = requests.filter((r) => (r.lifecycleStatus || "SUBMITTED") === q.status);
  if (q.studio) requests = requests.filter((r) => (r.assignedStudio || "") === q.studio);
  if (q.priority) requests = requests.filter((r) => (r.priority || "") === q.priority);
  if (q.identity) requests = requests.filter((r) => r.workspaceIdentity === q.identity);
  if (q.membership) requests = requests.filter((r) => (r.membershipEntitlement || "") === q.membership);
  if (q.project) requests = requests.filter((r) => (r.projectId || "") === q.project);
  res.json({ requests });
});

// Admin Production Jobs (Unified lifecycle & version engine)
apiRouter.get("/admin/production/jobs", requireAuth, requireAdmin(["super_admin", "admin"]), (req: AuthenticatedRequest, res: Response) => {
  const q = req.query as Record<string, string | undefined>;
  let jobs = db.getProductionJobs();
  if (q.status) jobs = jobs.filter((j) => (j.status || "SUBMITTED") === q.status);
  if (q.studio) jobs = jobs.filter((j) => (j.assignedStudio || "") === q.studio);
  if (q.priority) jobs = jobs.filter((j) => (j.priority || "") === q.priority);
  if (q.identity) jobs = jobs.filter((j) => j.identity === q.identity);
  if (q.search) {
    const s = q.search.toLowerCase();
    jobs = jobs.filter((j) =>
      j.serviceName.toLowerCase().includes(s) ||
      (j.customerName || "").toLowerCase().includes(s) ||
      (j.projectTitle || "").toLowerCase().includes(s)
    );
  }
  res.json({ jobs });
});

apiRouter.get("/admin/production/jobs/:jobId", requireAuth, requireAdmin(["super_admin", "admin"]), (req: AuthenticatedRequest, res: Response) => {
  const job = db.getProductionJobById(req.params.jobId);
  if (!job) return res.status(404).json({ error: "Production job not found" });
  res.json({ job });
});

apiRouter.put("/admin/production/jobs/:jobId", requireAuth, requireAdmin(["super_admin", "admin"]), (req: AuthenticatedRequest, res: Response) => {
  const job = db.getProductionJobById(req.params.jobId);
  if (!job) return res.status(404).json({ error: "Production job not found" });
  const updated = db.updateProductionJob(job.id, req.body || {});
  res.json({ job: updated });
});

apiRouter.post("/admin/production/jobs/:jobId/transition", requireAuth, requireAdmin(["super_admin", "admin"]), (req: AuthenticatedRequest, res: Response) => {
  const job = db.getProductionJobById(req.params.jobId);
  if (!job) return res.status(404).json({ error: "Production job not found" });
  ...
  if (status === "CLIENT_REVIEW") {
    try {
      db.addNotification(
        job.workspaceId,
        "Creative Deliverables Ready for Review",
        `New deliverables for '${job.serviceName}' are ready for your review in Project Studio.`,
        "info",
        job.projectId ? `/projects?id=${job.projectId}` : "/requests"
      );
    } catch (_err) {
...
apiRouter.post("/admin/production/jobs/:jobId/deliverables/:deliverableId/versions", requireAuth, requireAdmin(["super_admin", "admin"]), (req: AuthenticatedRequest, res: Response) => {
  const job = db.getProductionJobById(req.params.jobId);
  if (!job) return res.status(404).json({ error: "Production job not found" });
  const { title, previewUrl, fileUrl, fileSize, fileType, notes, status } = req.body || {};
  if (!previewUrl && !fileUrl) {
    return res.status(400).json({ error: "previewUrl or fileUrl is required" });
  }

  const result = db.addDeliverableVersion(job.id, req.params.deliverableId, {
    title,
  ...
  if (status === "client_review" || !status) {
    try {
      db.addNotification(
        job.workspaceId,
        "Deliverable Updated",
        `Version ${result.version.versionNumber} for '${result.version.title}' is ready for client review.`,
        "info",
        job.projectId ? `/projects?id=${job.projectId}` : "/requests"
      );
    } catch (_err) {
  ...
  res.status(201).json({ success: true, job: result.job, version: result.version });
});

apiRouter.post("/admin/production/jobs/:jobId/deliver", requireAuth, requireAdmin(["super_admin", "admin"]), (req: AuthenticatedRequest, res: Response) => {
  const job = db.getProductionJobById(req.params.jobId);
  if (!job) return res.status(404).json({ error: "Production job not found" });
  const { note } = req.body || {};
  const { job: updatedJob, createdAssets } = db.finalizeJobDelivery(job.id, {
  ...
  try {
    db.addNotification(
      job.workspaceId,
      "Studio Assets Delivered",
      `Final assets for '${job.serviceName}' have been delivered to your Project and Workspace Library.`,
      "success",
      job.projectId ? `/projects?id=${job.projectId}` : "/library"
    );
  } catch (_err) {
  ...
  res.status(201).json({ success: true, job: updatedJob, createdAssets });
});

apiRouter.post("/admin/production/jobs/:jobId/deliverables", requireAuth, requireAdmin(["super_admin", "admin"]), (req: AuthenticatedRequest, res: Response) => {
  const job = db.getProductionJobById(req.params.jobId);
  if (!job) return res.status(404).json({ error: "Production job not found" });
  const { title, category, format, specs, downloadAllowed } = req.body || {};
  if (!title) return res.status(400).json({ error: "Deliverable title is required" });

  const newDel: ProductionDeliverableRecord = {
    id: `del_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`,
    jobId: job.id,
    title: title.trim(),
    category: category || "artwork",
    format: format || "JPG",
    specs: specs || "3000x3000px 300DPI",
...
apiRouter.post("/admin/production/jobs/:jobId/revisions/:revisionId/resolve", requireAuth, requireAdmin(["super_admin", "admin"]), (req: AuthenticatedRequest, res: Response) => {
  const job = db.getProductionJobById(req.params.jobId);
  if (!job) return res.status(404).json({ error: "Production job not found" });

  const rev = job.revisions.find((r) => r.id === req.params.revisionId);
...
apiRouter.get("/admin/production/requests/:requestId", requireAuth, requireAdmin(["super_admin", "admin"]), (req: AuthenticatedRequest, res: Response) => {
  const found = db.getCreativeRequestById(req.params.requestId);
  if (!found) return res.status(404).json({ error: "Request not found" });
  res.json({ request: enrichProductionRequest(found) });
});

apiRouter.post("/admin/production/requests/:requestId/transition", requireAuth, requireAdmin(["super_admin", "admin"]), (req: AuthenticatedRequest, res: Response) => {
  const found = db.getCreativeRequestById(req.params.requestId);
  if (!found) return res.status(404).json({ error: "Request not found" });
  const to = (req.body || {}).to as string;
  const from = found.lifecycleStatus || "SUBMITTED";
  if (!to) return res.status(400).json({ error: "Transition target 'to' is required" });
  if (!((PRODUCTION_NEXT[from] || []).includes(to) || to === "CANCELLED")) {
...
apiRouter.post("/admin/production/requests/:requestId/assign", requireAuth, requireAdmin(["super_admin", "admin"]), (req: AuthenticatedRequest, res: Response) => {
  const found = db.getCreativeRequestById(req.params.requestId);
  if (!found) return res.status(404).json({ error: "Request not found" });
  const { assignedStudio, assignedProducer, assignedAdminId } = req.body || {};
  if (!assignedStudio) return res.status(400).json({ error: "assignedStudio is required" });
  const updated = db.updateCreativeRequestById(found.id, {
    assignedStudio,
    assignedProducer: assignedProducer || req.user!.fullName,
...
apiRouter.post("/admin/production/requests/:requestId/notes", requireAuth, requireAdmin(["super_admin", "admin"]), (req: AuthenticatedRequest, res: Response) => {
  const found = db.getCreativeRequestById(req.params.requestId);
  if (!found) return res.status(404).json({ error: "Request not found" });
  const { internalNotes, clientVisibleNotes } = req.body || {};
...
apiRouter.post("/admin/production/requests/:requestId/revisions", requireAuth, requireAdmin(["super_admin", "admin"]), (req: AuthenticatedRequest, res: Response) => {
  const found: any = db.getCreativeRequestById(req.params.requestId);
  if (!found) return res.status(404).json({ error: "Request not found" });
  const { reason, instructions } = req.body || {};
...
// Delivery: creates real assets in the customer's project/library, records the
// delivery dispatch, and moves the request into DELIVERY.
apiRouter.post("/admin/production/requests/:requestId/deliver", requireAuth, requireAdmin(["super_admin", "admin"]), (req: AuthenticatedRequest, res: Response) => {
  const found: any = db.getCreativeRequestById(req.params.requestId);
  if (!found) return res.status(404).json({ error: "Request not found" });
  const { assets, note } = req.body || {};
  if (!Array.isArray(assets) || assets.length === 0) {
    return res.status(400).json({ error: "At least one asset is required to deliver" });
  }
  const created: any[] = [];
  for (const a of assets) {
    if (!a.name || !a.url) continue;
    const asset = db.createAsset(found.workspaceId, {
      projectId: found.projectId,
      name: a.name,
      category: (a.category as any) || "artwork",
      url: a.url,
      size: Number(a.size) || 0,
      mimeType: a.mimeType || "image/png",
      tags: a.tags || ["delivery"],
      folderName: "Deliveries",
      metadata: {
        deliveryFromRequestId: found.id,
        approved: true,
        ...(a.metadata || {}),
      },
    } as any);
  ...
  if (created.length === 0) {
    return res.status(400).json({ error: "No valid assets in the delivery" });
  }
  db.addCreativeRequestDelivery(found.id, {
...
apiRouter.get("/admin/platform/plan-config", requireAuth, requireAdmin(["super_admin", "admin"]), (_req: AuthenticatedRequest, res: Response) => {
  const ps = db.getPlatformSettings();
  res.json({ planConfigs: ps.planConfigs || [] });
});

apiRouter.put("/admin/platform/plan-config", requireAuth, requireAdmin(["super_admin", "admin"]), (req: AuthenticatedRequest, res: Response) => {
  const { planConfigs } = req.body || {};
  if (!Array.isArray(planConfigs) || planConfigs.length === 0) {
    return res.status(400).json({ error: "planConfigs array is required" });
  }
  const ps = db.updatePlatformSettings({ planConfigs });
  res.json({ planConfigs: ps.planConfigs || [] });
});

apiRouter.get("/admin/platform/production-config", requireAuth, requireAdmin(["super_admin", "admin"]), (_req: AuthenticatedRequest, res: Response) => {
  const ps = db.getPlatformSettings();
  res.json({ productionConfig: ps.productionConfig || null });
});

apiRouter.put("/admin/platform/production-config", requireAuth, requireAdmin(["super_admin", "admin"]), (req: AuthenticatedRequest, res: Response) => {
  const { productionConfig } = req.body || {};
  if (!productionConfig || typeof productionConfig !== "object") {
    return res.status(400).json({ error: "productionConfig object is required" });
  }
  const ps = db.updatePlatformSettings({ productionConfig });
  res.json({ productionConfig: ps.productionConfig || null });
});
...
apiRouter.post("/workspaces/:workspaceId/campaigns/:campaignId/migrate", requireAuth, requireWorkspaceAccess, (req: AuthenticatedRequest, res: Response) => {
  const camp: any = db.getCampaignById(req.params.workspaceId, req.params.campaignId);
  if (!camp) return res.status(404).json({ error: "Campaign not found" });
  const status = camp.status === "active" ? "in-progress" : camp.status === "completed" ? "completed" : "planning";
  const project = db.createProject(req.params.workspaceId, {
    title: camp.title,
    description: camp.goal ? `Migrated from campaign "${camp.title}". Goal: ${camp.goal}` : `Migrated from campaign "${camp.title}".`,
    status,
    priority: "medium",
    migratedFromCampaignId: camp.id,
  } as any);
  res.status(201).json({ project, migratedFrom: camp.id });
});
