      ...
      await client.query(`
        ...
        LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
          SELECT EXISTS (
            ...
            WHERE m.workspace_id = ws_id AND (m.payload->>'userId')::text = kh_current_user_id()
          ) OR EXISTS (
    ...
    try {
      ...
      for (const key of ARRAY_COLLECTIONS) {
        ...
        for (const row of rows) {
          // Sessions are keyed by their opaque token (SessionRecord has no
          // dedicated `id` column), so fall back to the token to satisfy the
          // table's NOT NULL primary key. This prevents the whole snapshot
          // transaction from rolling back whenever a session is present.
          const rowId = key === "sessions" ? row.token : row.id;
          if (!rowId) {
            throw new Error(
              `Row in "${key}" is missing a stable id and cannot be persisted.`
            );
          }
          await client.query(
            `INSERT INTO "${this.table(key)}" (id, workspace_id, payload) VALUES ($1, $2, $3)`,
            [rowId, row.workspaceId || null, JSON.stringify(row)]
          );
        }
      }
...
export function setActiveStore(store: DatabaseStore): void {
  activeStore = store;
}
...
export function getDatabaseStore(): DatabaseStore {
  ...
  const engineRaw = (process.env.DB_ENGINE || "").toLowerCase().trim();
  const dbUrl = process.env.DATABASE_URL;

  // Use PostgreSQL only if DATABASE_URL is provided and engine is not explicitly "json"
  if (dbUrl && engineRaw !== "json") {
    try {
      activeStore = new PostgresStore(dbUrl);
      return activeStore;
    } catch (err: any) {
      console.warn(`[DB] PostgresStore setup failed (${err?.message}). Falling back to JSON store.`);
      activeStore = new JsonStore();
      return activeStore;
    }
  }

  // If engine was set to "json" or if DATABASE_URL is absent (standard dev/sandbox environment)
  if (engineRaw === "postgres" && !dbUrl) {
    console.warn(
      "[DB] DB_ENGINE=postgres requested but DATABASE_URL is not set. Using JSON fallback store in data/keedohub_db.json."
    );
  } else {
    console.info(
      "[DB] Using local JSON store (data/keedohub_db.json) for persistence."
    );
  }
