...
export type RadarSignalType = 
  ...
  | 'inactive_project'
  | 'deliverable_due'
  | 'empty_content_period'
  | 'missing_platform_coverage'
  | 'unused_asset'
  | 'content_stuck'
  | 'scheduled_content_conflict'
  | 'asset_not_connected'
  | 'asset_missing'
  | 'asset_awaiting_approval'
  | 'creative_deliverable_incomplete'
  | 'studio_request_awaiting_review'
  | 'quote_awaiting_approval'
  | 'project_awaiting_feedback'
  | 'revision_awaiting_production'
  | 'studio_deliverable_deadline'
  | 'studio_final_delivery_awaiting_approval';

export type RadarSignalSeverity = 'critical' | 'high' | 'medium' | 'low';

export type RadarSignalStatus = 
  | 'NEW'
  | 'ACKNOWLEDGED'
  | 'ACTIONED'
  | 'DISMISSED'
...
export type RadarSignalPriority = 'CRITICAL' | 'HIGH' | 'MEDIUM' | 'LOW';

export type RadarSignalEntityType = 
  | 'release'
  | 'campaign'
  | 'project'
  | 'task'
...
export interface RadarSignal {
  ...
  priority: RadarSignalPriority;
  title: string;
  explanation: string;
  affectedEntity: {
    type: RadarSignalEntityType;
    id: string;
    title?: string;
  };
  recommendedAction: {
    label: string;
    actionType: 'open_entity' | 'create_task' | 'generate_content' | 'request_studio' | 'ask_brain' | 'fix_readiness';
    targetTab?: ActiveTab;
    entityId?: string;
  }[];
  createdAt: string;
...
export interface RadarDigest {
  ...
  byCategory: {
    releases: RadarSignal[];
    campaigns: RadarSignal[];
    projects: RadarSignal[];
    content: RadarSignal[];
    studio: RadarSignal[];
  };
}
...
export interface RadarConfiguration {
  ...
  disabledSignalTypes: RadarSignalType[];
  customThresholds: {
    releaseApproachingDays: number;
    campaignApproachingDays: number;
    taskOverdueDays: number;
    readinessBlockerThreshold: number;
  };
}
