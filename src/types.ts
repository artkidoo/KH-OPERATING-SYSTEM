...
export type AssetCategory = 'cover' | 'audio' | 'image' | 'video' | 'document' | 'epk' | 'brand' | 'artwork' | 'motion' | 'working' | 'dsp' | 'social' | 'print' | 'merch' | 'copy' | 'web' | 'other';
...
export interface Project {
  ...
  workspaceId: string;
  releaseId?: string;
  /** @deprecated legacy link — projects no longer belong to campaigns. */
  campaignId?: string;
  /** Set when this project was migrated from a legacy campaign. */
  migratedFromCampaignId?: string;
  /** Logical sections inside the project (Step 4). Defaults per identity via defaultProjectSections(). */
  sections?: string[];
  title: string;
  name?: string;
  description: string;
...
export interface Campaign {
  /**
   * @deprecated Campaign-first architecture was removed in Phase 1.
   * Campaigns are read-only legacy data. New work belongs in Projects
   * (see src/domain/campaignMigration.ts). campaignId fields elsewhere
   * are optional legacy links only.
   */
  id: string;
  workspaceId: string;
  releaseId?: string;
...
export interface CreativeRequest {
  ...
  // ---- Phase 1 foundation (all optional for backward compatibility) ----
  projectId?: string;
  requesterId?: string;
  requestType?: string;
  title?: string;
...
// ============================================================
// PRODUCTION JOB ENGINE TYPES (Step 1-6)
// Supporting generic production lifecycle for all Studio Services
// ============================================================
...
export interface ProductionDeliverableVersion {
  id: string;
  ...
  uploadedBy: string;
  uploadedAt: string;
  status: 'internal' | 'client_review' | 'revision_requested' | 'approved' | 'delivered';
...
export interface ProductionDeliverable {
  ...
  versions: ProductionDeliverableVersion[];
}
...
export interface ProductionFinalDelivery {
  deliveredAt: string;
  deliveredBy: string;
...
export interface ProductionJob {
  ...
  finalDelivery?: ProductionFinalDelivery;
  createdAt: string;
  updatedAt: string;
}
...
export interface AdminUserSummary {
  ...
  recentActivity?: { id?: string; action: string; details?: string; entityType?: string; createdAt?: string; timestamp?: string }[];
  createdAt: string;
  lastLoginAt?: string;
}
