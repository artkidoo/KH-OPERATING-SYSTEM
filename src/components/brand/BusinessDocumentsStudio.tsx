...
export const BusinessDocumentsStudio: React.FC<{
  ...
  return (
    <div className="min-w-0 max-w-full space-y-8 overflow-hidden" id="business-documents-studio">
      <header className="relative overflow-hidden rounded-3xl border border-[var(--bento-border)] bg-gradient-to-br from-[var(--bento-card)] via-[var(--bento-card)] to-blue-500/10 p-6 sm:p-8">
        <div className="absolute -right-16 -top-20 h-56 w-56 rounded-full bg-blue-500/10 blur-3xl" />
        <div className="relative flex min-w-0 flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              <span className="flex items-center gap-2 rounded-full border border-blue-400/25 bg-blue-500/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-[.16em] text-blue-300"><Sparkles className="h-3.5 w-3.5" /> Business Documents Studio</span>
          ...
          <div className="flex min-w-0 shrink-0 flex-wrap gap-2">
            <button onClick={() => setShowFavoritesOnly((current) => !current)} className={`flex items-center gap-2 rounded-xl border px-3.5 py-2.5 text-xs font-bold transition ${showFavoritesOnly ? "border-amber-400/40 bg-amber-400/10 text-amber-300" : "border-[var(--bento-border)] bg-[var(--bento-card)] text-[var(--bento-muted)] hover:text-[var(--bento-text)]"}`}><Star className="h-4 w-4" fill={showFavoritesOnly ? "currentColor" : "none"} /> Favorites {favorites.length ? `(${favorites.length})` : ""}</button>
            <button onClick={() => setShowSaved((open) => !open)} className="flex items-center gap-2 rounded-xl border border-[var(--bento-border)] bg-[var(--bento-card)] px-3.5 py-2.5 text-xs font-bold text-[var(--bento-text)] hover:border-blue-400/40"><Save className="h-4 w-4" /> Saved {savedDocuments.length ? `(${savedDocuments.length})` : ""}</button>
          </div>
        </div>
      </header>
      ...
      <section aria-label="Template library" className="space-y-6">
        <div className="flex items-center justify-between gap-3">
          <div><p className="text-[11px] font-black uppercase tracking-[.18em] text-blue-400">Template library</p><h2 className="mt-1 text-2xl font-black text-[var(--bento-text)]">Choose your starting point</h2></div>
          <span className="hidden rounded-full bg-[var(--bento-card)] px-3 py-1.5 text-[11px] font-bold text-[var(--bento-muted)] sm:block">{filteredTemplates.length} of {availableTemplates.length} templates</span>
        </div>
        <div className="flex gap-2 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {(["all", ...businessDocumentTypeOrder] as ("all" | BusinessDocumentType)[]).map((type) => (
            <button key={type} onClick={() => setActiveType(type)} className={`whitespace-nowrap rounded-xl border px-3.5 py-2.5 text-xs font-bold transition ${activeType === type ? "border-blue-400/40 bg-blue-500/15 text-blue-300 shadow-lg shadow-blue-950/20" : "border-[var(--bento-border)] bg-[var(--bento-card)] text-[var(--bento-muted)] hover:text-[var(--bento-text)]"}`}>{type === "all" ? "All" : businessDocumentTypeLabels[type]}</button>
          ))}
        </div>
        {filteredTemplates.length === 0 ? <div className="rounded-2xl border border-dashed border-[var(--bento-border)] p-10 text-center text-sm text-[var(--bento-muted)]">No favorites in this view yet. Tap the star on any template to save it.</div> : <div className="grid min-w-0 grid-cols-1 items-stretch gap-5 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">{filteredTemplates.map((template) => <TemplateCard key={template.id} template={template} values={values} brandCore={brandCore} headingFont={headingFont} bodyFont={bodyFont} isFavorite={favorites.includes(template.id)} isSelected={template.id === selectedTemplate.id} onFavorite={() => toggleFavorite(template.id)} onPreview={() => setPreviewTemplate(template)} onUse={() => selectTemplate(template)} />)}</div>}
      </section>

      <section id="business-document-editor" className="scroll-mt-8 space-y-5 rounded-3xl border border-[var(--bento-border)] bg-[var(--bento-card)] p-4 sm:p-6">
        <div className="flex flex-col gap-4 border-b border-[var(--bento-border)] pb-5 lg:flex-row lg:items-center lg:justify-between">
          <div><div className="flex flex-wrap items-center gap-2"><Pencil className="h-4 w-4 text-blue-400" /><p className="text-[11px] font-black uppercase tracking-[.16em] text-blue-400">Editor</p><span className="rounded-full bg-blue-500/10 px-2 py-1 text-[10px] font-bold text-blue-300">{selectedTemplate.styleLabel || "Brand-ready style"}</span></div><h2 className="mt-1 text-2xl font-black text-[var(--bento-text)]">{selectedTemplate.name}</h2><p className="mt-1 text-xs text-[var(--bento-muted)]">{selectedTemplate.description}</p></div>
          <div className="flex flex-wrap gap-2"><button onClick={refreshBrandValues} className="flex items-center gap-2 rounded-xl border border-[var(--bento-border)] px-3 py-2 text-xs font-bold text-[var(--bento-muted)] hover:text-[var(--bento-text)]"><RefreshCw className="h-3.5 w-3.5" /> Refresh Brand OS</button><button onClick={() => exportDocument("json")} className="flex items-center gap-2 rounded-xl border border-[var(--bento-border)] px-3 py-2 text-xs font-bold text-[var(--bento-muted)] hover:text-[var(--bento-text)]"><Download className="h-3.5 w-3.5" /> JSON</button><button onClick={saveDocument} className="flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 text-xs font-black text-white shadow-lg shadow-emerald-950/25 hover:bg-emerald-500"><Save className="h-3.5 w-3.5" /> Save document</button></div>
        </div>
        <div className="grid gap-6 xl:grid-cols-[minmax(280px,.8fr)_minmax(460px,1.2fr)]">
          <div className="space-y-4">
            <div className="rounded-2xl border border-blue-400/20 bg-blue-500/5 p-3 text-xs leading-5 text-[var(--bento-muted)]"><span className="font-bold text-blue-300">Editable content, protected design.</span> Change the fields below while the selected template keeps its layout, typography, and brand styling.</div>
...
const TemplateCard: React.FC<{
  ...
  return <article className={`group flex min-w-0 h-full flex-col overflow-hidden rounded-2xl border bg-[var(--bento-card)] transition duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-blue-950/20 focus-within:ring-2 focus-within:ring-blue-400/40 ${isSelected ? "border-blue-400/50 ring-1 ring-blue-400/20" : "border-[var(--bento-border)]"}`}>
    <div className="relative aspect-[1.35/1] min-h-[190px] overflow-hidden bg-zinc-100 p-3 sm:p-4"><div className="pointer-events-none mx-auto h-full max-w-[330px] origin-top scale-[.78] sm:scale-[.84]"><DocumentCanvas template={template} values={values} brandCore={brandCore} headingFont={headingFont} bodyFont={bodyFont} accent={brandCore?.colorPalette?.[0]?.hex || styleAccent[getStyle(template)]} compact /></div><div className="absolute inset-x-3 top-3 flex items-start justify-between gap-2"><div className="flex min-w-0 flex-wrap gap-1.5"><span className="flex items-center gap-1.5 rounded-full bg-black/65 px-2.5 py-1 text-[10px] font-bold text-white backdrop-blur"><Icon className="h-3 w-3 shrink-0" /> {template.styleLabel?.split(" / ")[0] || "Brand-ready"}</span>{isRecommended && <span className="rounded-full bg-emerald-500/90 px-2.5 py-1 text-[9px] font-black text-white">For your industry</span>}</div><button aria-label={`${isFavorite ? "Remove" : "Add"} ${template.name} favorite`} onClick={onFavorite} className={`shrink-0 rounded-full p-2.5 backdrop-blur transition hover:scale-105 ${isFavorite ? "bg-amber-400 text-amber-950" : "bg-black/50 text-white hover:bg-black/75"}`}><Heart className="h-3.5 w-3.5" fill={isFavorite ? "currentColor" : "none"} /></button></div></div>
    <div className="flex flex-1 flex-col gap-3 p-4"><div><div className="flex items-start justify-between gap-2"><h3 className="min-w-0 text-base font-black leading-tight text-[var(--bento-text)]">{template.name}</h3>{isSelected && <span className="shrink-0 rounded-full bg-emerald-500/10 px-2 py-1 text-[9px] font-black uppercase tracking-wide text-emerald-400">In use</span>}</div><p className="mt-1.5 min-h-10 text-xs leading-5 text-[var(--bento-muted)]">{template.description}</p></div><div className="flex min-h-8 flex-wrap content-start gap-1.5">{(template.suitedIndustries || []).map((industry) => <span key={industry} className="rounded-md bg-[var(--bento-bg)] px-2 py-1 text-[9px] font-bold text-[var(--bento-subtle)]">{industry}</span>)}</div><div className="mt-auto flex gap-2 pt-1"><button onClick={onPreview} className="flex min-h-11 flex-1 items-center justify-center gap-1.5 rounded-xl border border-[var(--bento-border)] px-3 py-2.5 text-xs font-bold text-[var(--bento-muted)] transition hover:border-blue-400/40 hover:text-[var(--bento-text)]"><Eye className="h-3.5 w-3.5" /> Preview</button><button onClick={onUse} className="flex min-h-11 flex-1 items-center justify-center gap-1.5 rounded-xl bg-blue-600 px-3 py-2.5 text-xs font-black text-white transition hover:bg-blue-500">Use template <ChevronRight className="h-3.5 w-3.5" /></button></div></div>
  </article>;
};

const DocumentField: React.FC<{ field: BusinessDocumentField; value: string; onChange: (value: string) => void }> = ({ field, value, onChange }) => <label className="space-y-1.5"><span className="flex items-center justify-between gap-2 text-xs font-bold text-[var(--bento-text)]">{field.label}{field.required && <span className="text-rose-400">*</span>}{field.autoFill && <span className="text-[10px] font-normal text-emerald-400">Brand OS</span>}</span>{field.type === "textarea" ? <textarea value={value} onChange={(event) => onChange(event.target.value)} rows={field.key === "body" || field.key === "scope" || field.key === "lineItems" ? 5 : 3} placeholder={field.placeholder} className="w-full resize-y rounded-xl border border-[var(--bento-border)] bg-[var(--bento-bg)] px-3 py-2.5 text-xs text-[var(--bento-text)] placeholder:text-[var(--bento-subtle)] outline-none transition focus:border-blue-500" /> : field.type === "select" ? <select value={value} onChange={(event) => onChange(event.target.value)} className="w-full rounded-xl border border-[var(--bento-border)] bg-[var(--bento-bg)] px-3 py-2.5 text-xs text-[var(--bento-text)] outline-none focus:border-blue-500">{field.options?.map((option) => <option key={option}>{option}</option>)}</select> : <input type={field.type} value={value} onChange={(event) => onChange(event.target.value)} placeholder={field.placeholder} className="w-full rounded-xl border border-[var(--bento-border)] bg-[var(--bento-bg)] px-3 py-2.5 text-xs text-[var(--bento-text)] placeholder:text-[var(--bento-subtle)] outline-none transition focus:border-blue-500" />}</label>;
...
const PreviewModal: React.FC<{ template: BusinessDocumentTemplate; values: Record<string, string>; brandCore: any; headingFont: string; bodyFont: string; onClose: () => void; onUse: () => void }> = ({ template, values, brandCore, headingFont, bodyFont, onClose, onUse }) => <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={`${template.name} preview`}><div className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-3xl border border-[var(--bento-border)] bg-[var(--bento-card)] shadow-2xl"><div className="flex items-center justify-between border-b border-[var(--bento-border)] p-4 sm:p-5"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-blue-400">Template preview</p><h2 className="mt-1 text-lg font-black text-[var(--bento-text)]">{template.name}</h2></div><button onClick={onClose} className="rounded-xl p-2 text-[var(--bento-muted)] hover:bg-[var(--bento-bg)] hover:text-[var(--bento-text)]" aria-label="Close preview"><X className="h-5 w-5" /></button></div><div className="flex-1 overflow-auto bg-zinc-100 p-5 sm:p-10"><DocumentCanvas template={template} values={values} brandCore={brandCore} headingFont={headingFont} bodyFont={bodyFont} accent={brandCore?.colorPalette?.[0]?.hex || styleAccent[getStyle(template)]} /></div><div className="flex flex-wrap justify-end gap-2 border-t border-[var(--bento-border)] p-4"><button onClick={onClose} className="rounded-xl border border-[var(--bento-border)] px-4 py-2.5 text-xs font-bold text-[var(--bento-muted)]">Close</button><button onClick={onUse} className="rounded-xl bg-blue-600 px-4 py-2.5 text-xs font-black text-white hover:bg-blue-500">Use this template</button></div></div></div>;
