    ...
    try {
      const res = await fetch('/api/ai/brand-strategy', {
        ...
        body: JSON.stringify({
          ...
          targetAudience: brandCore?.audience?.primaryICP || 'Modern founders and scale-ups',
      ...
      if (json && json.data) {
        ...
        await saveBrandCore({
          ...
          voiceAndTone: {
            ...
            communicationPrinciples: ['Be direct', 'Quantify value', 'Avoid buzzwords'],
      ...
      {subSection === 'audience' && (
        <div className="bg-zinc-900/80 border border-zinc-800 rounded-2xl p-6 space-y-5">
          ...
          <div className="space-y-2">
            ...
            <input
              type="text"
              value={brandCore.audience?.primaryICP || ''}
              onChange={(e) =>
                saveBrandCore({
                  audience: {
                    ...(brandCore.audience || { primaryICP: '', targetSegments: [], painPoints: [], coreDesires: [] }),
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <textarea
                rows={4}
                value={(brandCore.audience?.painPoints || []).join('\n')}
                onChange={(e) =>
                  saveBrandCore({
                    audience: {
                      ...(brandCore.audience || { primaryICP: '', targetSegments: [], painPoints: [], coreDesires: [] }),
            ...
            <div className="space-y-2">
              ...
              <textarea
                rows={4}
                value={(brandCore.audience?.coreDesires || []).join('\n')}
                onChange={(e) =>
                  saveBrandCore({
                    audience: {
                      ...(brandCore.audience || { primaryICP: '', targetSegments: [], painPoints: [], coreDesires: [] }),
