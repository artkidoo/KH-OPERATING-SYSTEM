...
export const BrandOperatingEnvironment: React.FC<BrandOperatingEnvironmentProps> = ({
  ...
  // Real data helpers
  const currentObjective = (brandCore?.valueProposition as string) || 'Accelerate enterprise client acquisition and scale predictable recurring revenue.';
  
  const activeProjectsList = useMemo(() => {
    return projects.filter((p) => p.status === 'planning' || p.status === 'in-progress').slice(0, 5);
  }, [projects]);
  ...
  const [brandDNA, setBrandDNA] = useState({
    ...
    positioning: brandCore?.positioningStatement || '',
    businessCategory: brandCore?.industry || workspace?.genreOrNiche || '',
    audience: brandCore?.targetAudience || '',
