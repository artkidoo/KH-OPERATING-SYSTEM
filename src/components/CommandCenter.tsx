  ...
  return (
    <div className="space-y-6 pb-12 animate-fade-in font-['Plus_Jakarta_Sans']">
      ...
      <ActivationChecklist
        ...
        releasesCount={counts.releases}
        campaignsCount={counts.campaigns}
        assetsCount={counts.assets}
        contentCount={counts.contentItems}
      />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="p-4 rounded-2xl bg-[var(--bento-card)] border border-[var(--bento-border)] relative overflow-hidden group">
          ...
          <p className="mt-1 text-[11px] text-zinc-400 truncate">
            {counts.activeBlockers === 0 ? "Zero operational blockers detected" : `${counts.activeBlockers} active blockers pending`}
          </p>
        </div>
        ...
        >
          ...
          <div className="mt-2 flex items-baseline gap-2">
            ...
            <span className="text-sm font-bold text-zinc-200">{counts.studioRequests}</span>
            <span className="text-xs text-zinc-400">requests</span>
          </div>
          <p className="mt-1 text-[11px] text-zinc-400 truncate">
            {counts.assets} assets stored in Vault
          </p>
        </div>
      </div>
      ...
      {data.performancePulse && (
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-1 text-xs">
            {(data.performancePulse?.topInsight ? [data.performancePulse.topInsight] : []).slice(0, 2).map((sig: any, idx: number) => (
              <div
                key={idx}
            ...
            {data.performancePulse.topInsight && (
              <div className="p-2.5 rounded-xl bg-purple-950/30 border border-purple-500/30 text-purple-200 truncate flex items-center gap-1.5">
                ...
                <span className="truncate"><strong>Insight:</strong> {typeof data.performancePulse.topInsight === 'string' ? data.performancePulse.topInsight : (data.performancePulse.topInsight as any)?.title || (data.performancePulse.topInsight as any)?.label || 'No insight available'}</span>
              </div>
            )}
          </div>
        </div>
