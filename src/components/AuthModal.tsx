...
export function AuthModal({ isOpen, onClose, onSuccess, initialMode = "login", adminContext = false }: AuthModalProps) {
  ...
  const handleDemoLogin = async (type: "artist" | "brand" | "admin") => {
    ...
    try {
      ...
      setError(err.message || "Demo session failed. Please try again.");
    }
  };

  const identities: { id: IdentityType; label: string; icon: any; desc: string }[] = [
    { id: "artist", label: "Music Artist", icon: Music, desc: "Release asset kits, cover art, mastering & Studio production" },
    { id: "brand", label: "Brand & Business", icon: Building2, desc: "Brand identity, business documents & creative infrastructure" },
  ];
          ...
          <div className="space-y-3 pt-2">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-zinc-950 px-2 text-zinc-500 font-medium">Preview KeedoHub</span>
              </div>
            </div>
            ...
            <div className="grid gap-3">
              ...
              <button
                ...
                className="group relative flex items-start gap-4 p-4 rounded-xl border border-zinc-800 bg-zinc-900/60 hover:bg-zinc-800/60 hover:border-red-500/50 transition-all cursor-pointer text-left"
              >
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-purple-600 to-violet-700 flex items-center justify-center shrink-0 shadow-lg">
                  <Music className="w-5 h-5 text-white" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-white">Artist Demo</span>
                    <Sparkles className="w-4 h-4 text-amber-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
                  <p className="text-xs text-zinc-400 mt-0.5">Music releases, cover art, DSP pitching, content rollout</p>
                  <p className="text-[10px] text-amber-500/70 mt-1.5 font-medium">Includes sample project, release & assets</p>
                </div>
              </button>
              ...
              <button
                ...
                className="group relative flex items-start gap-4 p-4 rounded-xl border border-zinc-800 bg-zinc-900/60 hover:bg-zinc-800/60 hover:border-red-500/50 transition-all cursor-pointer text-left"
              >
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-600 to-cyan-700 flex items-center justify-center shrink-0 shadow-lg">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-zinc-400 mt-0.5">Brand identity, business documents, creative work, library</p>
                  <p className="text-[10px] text-amber-500/70 mt-1.5 font-medium">Includes sample project, brand core & documents</p>
                </div>
              </button>
            </div>

            <p className="text-[10px] text-zinc-600 text-center">Demo data is temporary and isolated from real accounts</p>
          </div>
        )}
