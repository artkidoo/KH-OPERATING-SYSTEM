...
export const ArtistContentBrain: React.FC<ArtistContentBrainProps> = ({ onNotify, openBriefModal }) => {
  ...
  const { createRelease, createCampaign, createContentItem } = useWorkspace();
  ...
  const handleCommitCampaign = async () => {
    ...
    try {
      ...
      const release = await createRelease({
        ...
        narrative: `Key Theme: ${keyTheme}. Target: ${targetAudience}. Tagline: ${rolloutPlan.tagline}`
      });
      ...
      const campaign = await createCampaign({
        ...
        budget: 2500,
        currency: "USD",
        platforms: ["TikTok", "Instagram Reels", "Spotify Editorial", "YouTube Shorts", "WhatsApp Superfans"]
      });
        ...
        for (const phase of rolloutPlan.phases) {
          for (const action of phase.actions.slice(0, 3)) {
            ...
            await createContentItem({
              ...
              title: `${act.contentType || act.format || 'Promo'}: ${(act.concept || act.action || '').slice(0, 40)}...`,
              format: (act.contentType || act.format || "post").toLowerCase().includes("video") ? "short-video" : "social-post",
              platform: act.platform || "instagram",
              scheduledDate: releaseDate,
              status: "draft",
              content: act.concept || act.action || "",
              caption: act.captionHook || act.hookIdea || act.audioDirective || "",
              audioReference: "Official Master Audio",
